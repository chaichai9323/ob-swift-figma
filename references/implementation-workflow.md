...
- Absence of a BaseCell section is valid. If no heading normalizes to `BaseCell`, skip BaseCell Figma evidence checks and do not inspect, create, replace, or modify `GeneralOBBaseCell`, `GeneralOBVC.nextButton`, `GeneralOB/icon_back`, or `Assets.xcassets/GeneralOB/Contents.json` for BaseCell work; continue with the page manifest and selected dispatch mode
...
- when a BaseCell section is present, process it in the parent worktree before dispatching any page subagent because it owns shared onboarding foundations
...
- when a BaseCell section is present, the parent completes and verifies its shared-foundation update for `GeneralOBBaseCell`, `GeneralOBVC.nextButton`, and `GeneralOB/icon_back` before dispatching the first page subagent; when absent, the parent skips that update and gate entirely; it never assigns BaseCell work to a page agent or adds a page status marker
...
- when no BaseCell section exists, confirmation that it was treated as optional and that no BaseCell evidence check or shared-target edit was performed
