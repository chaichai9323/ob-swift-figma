...
- `assets/GeneralOB` is copied somewhere outside the engineering project directory or app source root, so the app cannot compile against it.
...
- If the required `GeneralOB` folder or any required path is missing, copy the skill-bundled `assets/GeneralOB` scaffold into the engineering project directory as `GeneralOB`, add copied Swift files to the primary app target's Compile Sources or equivalent generated target source list, re-run the required-path check, verify target membership, and do not continue with stub files.
